...
export function DealCard({ deal, isFavorite: initialFav = false }: { deal: DealCardData; isFavorite?: boolean }) {
  const { user } = useAuth();
  ...
  const gallery = (deal.images && deal.images.length > 0 ? deal.images : (deal.image_url ? [deal.image_url] : [])).filter(Boolean);
  ...
  return (
    <Link
      to="/chollo/$slug" params={{ slug: deal.slug }}
    ...
    >
      <div className="relative aspect-[4/3] bg-surface-700 overflow-hidden border-b border-surface-700">
          ...
          <>
            ...
            {isExpired && (
              <div className="absolute inset-0 bg-surface-900/60 flex items-center justify-center z-10">
                <span className="bg-alert-red text-white font-mono font-bold text-[10px] px-2 py-1 -rotate-6 border border-white/40">CADUCADO</span>
              </div>
            )}
            {hasMultiple && !isExpired && (
              <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-1 z-10">
                {gallery.map((_, i) => (
                  <span key={i} className={`size-1.5 rounded-full transition-all ${i === imgIdx ? "bg-cyan-glow w-4" : "bg-white/60"}`} />
                ))}
              </div>
            )}
          </>
        ) : (
          <div className="w-full h-full flex items-center justify-center text-muted-foreground font-mono text-xs">SIN_IMAGEN</div>
        )}
        ...
        <button
          onClick={toggleFav} disabled={loading}
        ...
        >
          <Heart className={`size-4 transition-colors ${fav ? "fill-cyan-glow text-cyan-glow" : "text-foreground"}`} />
        </button>
      </div>

      <div className="p-4 sm:p-5 flex flex-col flex-1">
        <div className="flex justify-between items-center mb-3 font-mono text-xs">
          <span className="text-cyan-glow truncate">@{deal.store?.slug?.toUpperCase() ?? "STORE"}</span>

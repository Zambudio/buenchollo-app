    ...
    <header className="border-b border-surface-700 bg-surface-900/95 sticky top-0 z-50 backdrop-blur-md">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
        ...
        <form onSubmit={onSearch} className="hidden md:flex flex-1 max-w-md">
          <div className="w-full flex items-center bg-surface-800 border border-surface-700 px-3 py-1.5 focus-within:border-cyan-glow focus-within:glow-cyan transition-all">
            ...
            <label htmlFor="header-search" className="sr-only">Buscar chollo</label>
            <input
              id="header-search"
              value={q} onChange={(e) => setQ(e.target.value)}
              type="text" placeholder="BUSCAR_CHOLLO..."
            ...
            />
            <button type="submit" aria-label="Buscar" className="text-cyan-glow hover:text-foreground transition-colors">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          {user ? (
            <>
              <Link to="/notificaciones" className="relative p-2 hover:text-cyan-glow transition-colors" title="Notificaciones">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-52">
                  <DropdownMenuItem asChild><Link to="/perfil">Mi perfil</Link></DropdownMenuItem>
                  <DropdownMenuItem asChild><Link to="/favoritos">Favoritos</Link></DropdownMenuItem>
                  <DropdownMenuItem asChild><Link to="/alertas">Alertas</Link></DropdownMenuItem>
                  {isAdmin && (
                    <>
            ...
            <Link to="/login" className="font-mono text-xs sm:text-sm hover:text-cyan-glow transition-colors whitespace-nowrap">
              [ ACCEDER ]
            </Link>
          )}
      ...
      {mobileOpen && (
        <div className="md:hidden border-t border-surface-700 px-4 py-4 space-y-3 bg-surface-800">
          <form onSubmit={onSearch} className="flex items-center bg-surface-900 border border-surface-700 px-3 py-2">
            <span className="text-cyan-glow font-mono mr-2 text-sm">&gt;</span>
            <label htmlFor="header-search-mobile" className="sr-only">Buscar chollo</label>
            <input id="header-search-mobile" value={q} onChange={(e) => setQ(e.target.value)} placeholder="BUSCAR..."
              className="bg-transparent outline-none w-full text-sm font-mono uppercase" />
          </form>
          <Link to="/explorar" className="block font-mono text-sm hover:text-cyan-glow" onClick={() => setMobileOpen(false)}>[ EXPLORAR ]</Link>
          <Link to="/categorias" className="block font-mono text-sm hover:text-cyan-glow" onClick={() => setMobileOpen(false)}>[ CATEGORÍAS ]</Link>
          {user && <Link to="/alertas" className="block font-mono text-sm hover:text-cyan-glow" onClick={() => setMobileOpen(false)}>[ ALERTAS ]</Link>}
        </div>
      )}
    </header>
    </>

...
export function HomeFilterTabs({ value, onChange }: Props) {
  return (
    <Tabs value={value} onValueChange={(v) => onChange(v as HomeFilterKey)}>
      <TabsList className="inline-flex h-auto items-center gap-1 bg-surface-800 border border-surface-700 rounded-full p-1">
        {OPTIONS.map(({ value: v, label, icon: Icon }) => (
          <TabsTrigger
            key={v}
            value={v}
            className="font-mono text-xs uppercase tracking-wide rounded-full px-4 py-2 gap-1.5 text-muted-foreground data-[state=active]:bg-cyan-glow data-[state=active]:text-surface-900 data-[state=active]:font-bold data-[state=active]:shadow-none"
          >

...
export function ShareBox({ url, title, price }: ShareBoxProps) {
  ...
  const btn =
    "inline-flex items-center justify-center gap-2 border border-surface-700 hover:border-cyan-glow hover:text-cyan-glow bg-surface-900 px-3 py-2.5 font-mono text-xs uppercase transition-colors";
  ...
  return (
    <section className="mt-8 bg-surface-800 border border-surface-700 p-5">
      ...
      <div className="flex flex-col sm:flex-row gap-2 mb-3">
        ...
        <button
          onClick={copy}
          className={`${btn} ${copied ? "border-cyan-glow text-cyan-glow" : ""}`}
        >
          {copied ? (
            <>

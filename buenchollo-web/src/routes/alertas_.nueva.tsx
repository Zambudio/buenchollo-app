...
export const Route = createFileRoute("/alertas_/nueva")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Crea una alerta para recibir avisos cuando aparezcan chollos de tus productos favoritos." },
...
function NewAlertPage() {
  ...
  const hasPreview = keyword || selectedCatName || selectedStoreName || brand || maxPrice || minDiscount;
  const inputCls = "w-full bg-surface-900 border border-surface-700 px-3 py-2.5 font-mono text-sm outline-none focus:border-cyan-glow transition-colors";

  return (
    <Layout>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10 sm:py-14">
        <Link to="/alertas" className="font-mono text-xs text-muted-foreground hover:text-cyan-glow transition-colors">
          &lt; MIS ALERTAS
        </Link>
        ...
        <section className="mt-8">
          <div className="flex items-center gap-3 mb-5">
            ...
            <div className="font-mono text-xs text-cyan-glow uppercase tracking-widest">&gt; RADAR_ALERTAS</div>
          </div>
        ...
        <form onSubmit={submit} className="mt-7 space-y-7">
          ...
          <div>
            <p className="text-sm text-muted-foreground mb-3">Descubre las mejores elecciones de la comunidad</p>
            <button
              type="button"
            ...
            {showAdvanced && (
              <div className="px-4 pb-4 space-y-4 border-t border-surface-700 pt-4">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block font-mono text-[10px] uppercase text-cyan-glow mb-2">Marca</label>
                    <input value={brand} onChange={(e) => setBrand(e.target.value)} className={inputCls} placeholder="Apple, Sony, Samsung..." />
                  </div>
                </div>

                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block font-mono text-[10px] uppercase text-cyan-glow mb-2">Precio maximo</label>
                    <input type="number" min="0" value={maxPrice} onChange={(e) => setMaxPrice(e.target.value)} className={inputCls} placeholder="sin limite" />
                  </div>
                  <div>
                    <label className="block font-mono text-[10px] uppercase text-cyan-glow mb-2">Descuento minimo</label>

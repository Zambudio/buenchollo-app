...
export const Route = createFileRoute("/login")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Accede a tu cuenta de BuencholloTech para gestionar alertas, favoritos y notificaciones." },
      { property: "og:title", content: "Acceder · BuencholloTech" },
      { property: "og:description", content: "Accede a tu cuenta de BuencholloTech para gestionar alertas, favoritos y notificaciones." },
      { property: "og:url", content: "https://buenchollotech.lovable.app/login" }, { name: "robots", content: "noindex, nofollow" },
    ],
...
function LoginPage() {
  ...
  const onGoogle = async () => {
    ...
    if (result.error) { toast.error("No se pudo iniciar sesión con Google"); return; }
    ...
    <div className="min-h-dvh flex items-center justify-center bg-background px-4 py-12">
      <div className="w-full max-w-md">
        ...
        <div className="bg-surface-800 border border-surface-700 p-6 sm:p-8">
          ...
          <p className="text-sm text-muted-foreground mb-6">Bienvenido de vuelta. Vamos a interceptar más chollos.</p>
          ...
          <p className="text-center text-sm text-muted-foreground mt-6">
            ¿No tienes cuenta? <Link to="/registro" className="text-cyan-glow hover:underline">Regístrate</Link>
          </p>
        </div>
      </div>
    </div>

...
export const Route = createFileRoute("/alertas")({
  ...
  head: () => ({
    meta: [
      { title: "Mis alertas · BuenChollo Tech" },
      {
        name: "description",
        content:
          "Gestiona tus radares de precios. Recibe avisos cuando aparezca un chollo que cumpla tus criterios.",
      },
      { property: "og:title", content: "Mis alertas · BuenChollo Tech" },
        ...
        {loading ? (
          ...
          <div className="bg-surface-800 border border-surface-700 p-16 text-center">
            ...
            <p className="text-muted-foreground text-sm mb-6">
              Crea una alerta y te avisamos cuando aparezca un chollo que te interese.
            </p>
            <Link
              to="/alertas/nueva"
              className="inline-flex items-center gap-2 bg-cyan-glow text-surface-900 font-mono text-xs font-bold px-5 py-3 hover:bg-foreground transition-colors"
            >
              ...
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-2">
                    <div
                      className={`size-2 rounded-full shrink-0 ${a.is_active ? "bg-cyan-glow animate-pulse" : "bg-surface-600"}`}
                    />
                    <h2 className="font-bold text-base truncate">{a.name}</h2>
                    {!a.is_active && (
                      <span className="font-mono text-[10px] uppercase bg-surface-700 text-muted-foreground px-2 py-0.5 shrink-0">
                        pausada
                      </span>
                    )}
                  </div>
                  <div className="flex flex-wrap gap-2">
                    ...
                    {(a.min_price != null || a.max_price != null) && (
                      <span className="flex items-center gap-1 text-[11px] font-mono bg-surface-900 border border-surface-600 px-2 py-0.5 text-foreground">
                        ...
                        {a.min_price != null ? `${a.min_price}` : "0"}–
                        {a.max_price != null ? `${a.max_price}` : "∞"}
                      </span>
                    )}
                    {a.min_discount != null && (
                      <span className="flex items-center gap-1 text-[11px] font-mono bg-surface-900 border border-surface-600 px-2 py-0.5 text-foreground">

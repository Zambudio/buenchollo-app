...
class DealComment(Base):
    ...
    parent_id: Mapped[str | None] = mapped_column(Uuid(as_uuid=False), ForeignKey("deal_comments.id", ondelete="CASCADE"), nullable=True)
...
class CommentVote(Base):
    ...
    id: Mapped[str] = mapped_column(Uuid(as_uuid=False), primary_key=True, server_default=func.gen_random_uuid())
    comment_id: Mapped[str] = mapped_column(Uuid(as_uuid=False), ForeignKey("deal_comments.id", ondelete="CASCADE"), nullable=False)
    user_id: Mapped[str] = mapped_column(Uuid(as_uuid=False), nullable=False)
